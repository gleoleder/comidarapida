...
let usuarioAutorizado = false; // Indica si el usuario está en la lista de autorizados
let nombreUsuario = '';    // Nombre del usuario desde la lista de autorizados
let rolUsuario = '';       // Rol del usuario (admin, cajero, etc.)
...
function handleTokenResponse(resp) {
    ...
    // Obtener email y validar usuario antes de cargar datos
    validarYCargarDatos();
}
...
function logoutGoogle() {
    ...
    hideAccessDenied(); // Ocultar mensaje de acceso denegado si existe
    showEmptyState();
    clearStats();
...
// ╔════════════════════════════════════════════════════════════════════════════╗
// ║                    SECCIÓN 6.1: VALIDACIÓN DE USUARIOS AUTORIZADOS          ║
// ╠════════════════════════════════════════════════════════════════════════════╣
// ║  Funciones para verificar si el email del usuario está en la lista de      ║
// ║  usuarios autorizados en Google Sheets antes de permitir el acceso         ║
// ╚════════════════════════════════════════════════════════════════════════════╝
...
async function validarYCargarDatos() {
    try {
        ...
        // Usuario autorizado - cargar todos los datos
        hideAccessDenied();
...
async function verificarUsuarioAutorizado(email) {
    try {
        ...
        // Si la hoja está vacía (solo encabezados), es primera vez
        if (rows.length === 0) {
            console.log('⚠️ La hoja de usuarios está vacía, mostrando opción de configurar');
            showAccessDenied(`No hay usuarios registrados aún`, true);
            return false;
        }
        
        // Buscar el email en la lista (columna A)
        // Estructura esperada: Email | Nombre | Rol | Activo
        for (const row of rows) {
            const emailLista = (row[0] || '').toString().trim().toLowerCase();
            const nombre = row[1] || '';
            const rol = row[2] || 'usuario';
            const activo = (row[3] || 'TRUE').toString().toUpperCase().trim();
            
            // Comparar emails (ignorando mayúsculas/minúsculas)
            if (emailLista === email.toLowerCase() && activo === 'TRUE') {
                usuarioAutorizado = true;
                nombreUsuario = nombre;
                rolUsuario = rol;
                console.log(`✅ Usuario autorizado: ${nombre} (${rol})`);
                return true;
            }
        }
        
        // Email no encontrado o no activo
        usuarioAutorizado = false;
        nombreUsuario = '';
        rolUsuario = '';
        ...
        // Si la hoja no existe, mostrar mensaje de configuración
        if (e.result && e.result.error) {
            const errorMsg = e.result.error.message || '';
            if (errorMsg.includes('Unable to parse range') || errorMsg.includes('not found') || e.result.error.code === 400) {
                console.log('⚠️ La hoja Usuarios_Autorizados no existe');
                showAccessDenied(`La hoja "Usuarios_Autorizados" no existe`, true);
                return false;
            }
        }
...
/**
 * Muestra el mensaje de acceso denegado y bloquea la interfaz
 * @param {string} mensaje - Mensaje a mostrar
 * @param {boolean} showSetup - Mostrar botón de configuración (para primer uso)
 */
function showAccessDenied(mensaje, showSetup = false) {
    usuarioAutorizado = false;
    
    // Actualizar el email mostrado con indicador de no autorizado
    const emailEl = document.getElementById('userEmail');
    if (emailEl) {
        emailEl.innerHTML = `<span style="color: #ff6b6b;">🚫 ${emailUsuario}</span>`;
    }
    
    // Crear botón de configuración si es necesario
    let setupButton = '';
    if (showSetup) {
        setupButton = `
 ...
 * SOLO se ejecuta si el usuario está autorizado
 */
async function loadAllDataFromSheets() {
    ...
    if (!usuarioAutorizado) {
        console.log('⚠️ Usuario no autorizado, no se cargarán datos');
        return;
    }
...
async function setupGoogleSheet() {
    ...
    try {
        ...
        // Si es la primera vez, agregar el usuario actual como administrador
        if (emailUsuario) {
            const usuariosExistentes = await gapi.client.sheets.spreadsheets.values.get({
                spreadsheetId: SPREADSHEET_ID,
                range: SHEETS.USUARIOS_AUTORIZADOS + '!A2:A100'
            });
            
            const emailsExistentes = (usuariosExistentes.result.values || []).flat().map(e => e.toLowerCase());
            
            if (!emailsExistentes.includes(emailUsuario.toLowerCase())) {
                await gapi.client.sheets.spreadsheets.values.append({
                    spreadsheetId: SPREADSHEET_ID,
                    range: SHEETS.USUARIOS_AUTORIZADOS + '!A:D',
                    valueInputOption: 'RAW',
                    insertDataOption: 'INSERT_ROWS',
                    resource: { 
                        values: [[emailUsuario, 'Administrador', 'admin', 'TRUE']] 
                    }
                });
                
                // Marcar como autorizado ya que se agregó a sí mismo
                usuarioAutorizado = true;
                nombreUsuario = 'Administrador';
                rolUsuario = 'admin';
                
                showToast('Te agregaste como administrador', 'success');
            }
        }
        ...
        // Recargar datos si el usuario ahora está autorizado
        if (usuarioAutorizado && !dataLoaded) {
            loadAllDataFromSheets();
        }
    } catch (e) {
        hideLoading();
